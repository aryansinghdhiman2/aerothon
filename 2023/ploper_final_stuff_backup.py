...
def concentric_contours(contours, hierarchy):
    detected_contours = []
    for i, contour in enumerate(contours):
        if cv.contourArea(contour) > 300 and hierarchy[0][i][2] != -1:
            detected_contours.append(contour)
            # till this we have all the parents with the kids
            # now we want their kids with no kids

    child_contours = []  # holds all the children of the parents
    for i, prnt_contour in enumerate(detected_contours):
        prnt_index = contours.index(prnt_contour)
        child_index = hierarchy[0][prnt_index][2]
        child_contour = contours[child_index]
        child_contours.append(child_contour)

    child_with_nochild = [
        x for x in child_contours if x not in detected_contours]

    detected_contours += child_with_nochild
...
def red_detection(frame, hsv_frame):
    ...
    final_contour_array = concentric_contours(contours, hierarchy)

    centers_list = []
    for contour in final_contour_array:
...
while True:
    ...
    print(frame_center)
    cv.circle(frame, frame_center, 5, (0,255,0), -1)

    blurred_frame = cv.GaussianBlur(frame, (15, 15), 9, 9)
        ...
        for data in result:
            ...
            if((vehicle.mode == GUIDED or vehicle.mode == AUTO)):
                ...
                if(data['color'] == 'target'):
                    descendAndRelease(vehicle,controller,adjusted_center[0],adjusted_center[1])
                    pass
            detected = True

...
def handle_first_alignment(args):
    ...
    match args["alignment_state"]:
        case 0: # Initial Alignment
            goto_center(vehicle,center[0],center[1],lat,lon,alt,heading)
            time.sleep(2)
            socketio.emit(client_channel_str,1)
        case 1: # 2nd Alignment
            align_at_center(vehicle,center[0],center[1],lat,lon,alt,heading)
            time.sleep(2)
            socketio.emit(client_channel_str,2)
        case 2:
            move_to_center_image_coords_with_custom_loc(vehicle,center[0],center[1],lat,lon,alt,heading)
            time.sleep(2)

...
async def run_tracker_in_thread(model_name, filename):
    ...
    out = cv2.VideoWriter('./TestResults/output_76.avi',
                          fourcc, 20.0, (640,  480))
    # cam = cv2.VideoCapture(
    #     "filesrc location=/mnt/c/Users/91798/Desktop/new_aerothon/aerothon/2024/Kafka/output9.avi ! decodebin ! videoconvert ! appsink", cv2.CAP_GSTREAMER)
    time.sleep(3)
    ...
    while True:
        ...
        results = model(frame, conf=0.60, iou=0.75)
        ...
        for r in results: 
            ...
            print(f"Aligment STate {alignment_state}")
            frame_cnt += 1
            ...
            color = (255,0,0)
            thickness = 2
            cv2.line(annotated_frame, (frame_width // 2 - 20, frame_height // 2), (frame_width // 2 + 20, frame_height // 2), color, thickness)

            cv2.line(annotated_frame, (frame_width // 2, frame_height // 2 - 20), (frame_width // 2, frame_height // 2 + 20), color, thickness)
            ...
            with open("./json_output.jsonl", "a") as outfile:
                ...
                for score, label, box in zip(res["scores"], res["labels"], res["boxes"]):
                    ...
                    if label == 1:
                        ...
                        print("PRINTING THE ADJUSTED CENTER")
